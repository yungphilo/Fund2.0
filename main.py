...
#from get import getGegu, getJijin
...
if __name__ == '__main__':
    while True:
        ...
        tb_gg = pt.PrettyTable()
        tb_gg.field_names = ["GP名称", "open", "old", "cur", "top","bottom","%"]
        for i in ggc:
            gg,ggu = getGegu(i.strip())
            if gg != False:
                ggcode = i
                name, openv, old, cur, top,	bottom,	Dif = gg['gegu_name'], gg['gegu_jinkai'], gg['gegu_zuoshou'], gg['gegu_xianjia'], gg['gegu_jingao'], gg['gegu_jindi'], gg['gegu_diff']
                Dif, cur = copareGegu(Dif, cur)
                try:
                    #cur = copareGegu(cur, flag=0)
                    cur = float(str(ggu).split('"')[1].split(',')[4])
                    # name =
                    open = float(str(ggu).split('"')[1].split(',')[1])
                    old = float(str(ggu).split('"')[1].split(',')[2])
                    top = float(str(ggu).split('"')[1].split(',')[6])
                    bottom  = float(str(ggu).split('"')[1].split(',')[5])
                    dif =   float(str(ggu).split('"')[1].split(',')[7])
                    #Dif = copareGegu(Dif,flag=0)
                    ggzd = 0
                    cha = cur - old
                    if cha > 0:
                        ggzd = cha / old
                        ...
                        ggzd = -(-cha / old)
                    ggzd = str(round(ggzd * 100, 2)) + '%'
                    ggzd = copareGegu(ggzd, 2)
                    tb_gg.add_row([name, openv, old, cur, top,	bottom,	Dif])
                except:
                    ggzd = '-'
                    ggzd = '-'
                    tb_gg.add_row([name, openv, old, cur, top,	bottom,	Dif])
